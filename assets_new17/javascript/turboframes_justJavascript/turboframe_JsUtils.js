...
const JsUtils = (function() {
    ...
    return {
        ...
        // **   Clone Extent   ** //
        ////////////////////////////
        // JsUtils.deepExtend(true, {}, objA, objB);
        // JsUtils.deepExtend({}, objA, objB);
        ...
        // **   HTML   ** //
        ////////////////////
        // JsUtils.setHTML(document.getElementById(""), str)
        setHTML: function(el, html) {
            el.innerHTML = html;
        },
        // JsUtils.getHTML(document.getElementById(''));
        getHTML: function(el) {
            if (el) {
                return el.innerHTML;
            }
        },
        // JsUtils.val(document.getElementById(''));
        val: function(el) {
        ...
        // JsUtils.wrap(document.getElementById(" "), 'li')
        wrap: function(el, elementTag) {
            const wrappingElement = document.createElement(elementTag);
            el.replaceWith(wrappingElement);
            return wrappingElement.appendChild(el);
        },
        /////////////////////////////
        // **     Strings      ** //
        ////////////////////////////
        // JsUtils.datePad(date.getMonth() + 1)
        datePad: function(num) {
            return num > 9 ? num : "0" + num;
        },
        // JsUtils.parseJson( JSON )
        ...
        // * 【使用模板來實現一個簡單的數據綁定】
        // * 實現簡單的數據綁定： @{name）, @(role)
        // * data : let data = { name : 'BarrY', role : '鑽石會員' };
        // * str  : let str = '歡迎 @(name)，等級： @(role) 光臨本站！';
        // * @ Param Data  需要綁定的數據對象，是一個 Json 格式的數據，Json = {name : 'BarrY', age : 18}
        // * @ let inner = JsUtils.formateString( str , data ); 
        // *   JsUtils.('[data-li="true"]').innerHTML = inner;
        stringBind: function(srting, data) {
            // return srting.replace(/@\((\w+)\)/g, function(match, key) {
            //   return typeof data[key] === 'undefined' ? '' : data[key];
            // return srting.replace(/@\((\w+)\)@/gi, function(match, key) { // @(val)@ return typeof data[key] === "undefined" ? "" : data[key]; });
            // }),
        },

        // JsUtils.numOfComma(num)
        ...
        // let arr = [100,200 ,300];
        // let arrPer = JsUtils.countPercentage(arr)
        countPercentage: function(countArray) {
            let num = eval(countArray.join('+'));
            let resultArray = [];
            for (let i = 0; i < countArray.length; i++) {
                let val = Math.floor((countArray[i] / num) * 100) + "%";
        ...
        // let el = document.querySelector('.t-link').textContent;
        // let checked = JsUtils.startsWith(el , '1' , true)
        ...
        // 判定目標字符串是否位於原字符串的末尾
        // Target 目標字符串 || Str 父字符串 || Ignorecase 是否忽略大小寫
        // JsUtils.endWith(target , str , true)
        // let el = document.querySelector('.t-link').textContent;
        // let checked = JsUtils.endWith(el , '1' , true)
