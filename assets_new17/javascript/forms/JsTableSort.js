...
// Table Sort
const JsTableSort = function(testingTableSortJS = false, domDocumentWindow = document) {
  ...
  for (let table of getTagTable) {
    if (table.getAttribute('data-tu-sort', true)) {
  ...
  function makeEachColumnSortable(th, columnIndex, tableBody, sortableTable) {
    ...
    let tableArrows = sortableTable.getAttribute('data-table-arrows', true);
    console.log(tableArrows)
    const arrowUp = " ▲";
    const arrowDown = " ▼";
...
if (
  ...
  document.addEventListener("DOMContentLoaded", JsTableSort, false);
}
if (typeof module == "object") {
  module.exports = JsTableSort;
}