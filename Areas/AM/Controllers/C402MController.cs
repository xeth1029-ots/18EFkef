...
namespace WKEFSERVICE.Areas.AM.Controllers
{
    public class C402MController : WKEFSERVICE.Controllers.BaseController
    {
        ...
        public ActionResult Index(C402MFormModel form)
        {
            ...
            // 查詢 主資料 Teacher
            AMDAO dao = new AMDAO();
            List<Teacher> lstTeachers = dao.GetRowList(new Teacher()).ToList(); //師資數
            ...
            try
            {
                ...
                for (int tmpUNITID = 1; tmpUNITID <= 5; tmpUNITID++)
                {
                    ...
                    var tmpTeachers_list = tmpTeachers.Select(x => x.IDNO).Distinct().ToList();  // 抓一個老師 IDNO 清單出來查詢用
                    int iRow = tmpIdx + tmpUNITID;
                    ...
                    if (form.PlanTypeCode == "0" || form.PlanTypeCode == "1")
                    {
                        ...
                        var list_D_Class = dao.QueryForListAll<tbTransData_D_Class>("AM.Get_TransData_D_Class_TeachingHours", HtACYear).Where(x => tmpTeachers_list.Contains(x.TeacherID)).ToList();
                        ...
                        foreach (var rowD in list_D_Class.Where(x => x.JobAbilityCode == "DC"))
                        {
                            ...
                            // 列出滿意的學員
                            var list_D_Satisfy_Good = list_D_Satisfy.Where(x => (x.Q1.ToDouble() + x.Q2.ToDouble() + x.Q3.ToDouble() + x.Q4.ToDouble() + x.Q5.ToDouble()
                        ...
                        foreach (var rowD in list_D_Class.Where(x => x.JobAbilityCode == "BC"))
                        {
                            ...
                            var list_D_Satisfy_Good = list_D_Satisfy.Where(x => (x.Q1.ToDouble() + x.Q2.ToDouble() + x.Q3.ToDouble() + x.Q4.ToDouble() + x.Q5.ToDouble()
                            + x.Q6.ToDouble() + x.Q7.ToDouble() + x.Q8.ToDouble() + x.Q9.ToDouble() + x.Q10.ToDouble() + x.Q11.ToDouble()) / 11 >= 4).ToList();
                            Good_BC_Sum = Good_BC_Sum + list_D_Satisfy_Good.ToCount(); //整體
                            Fill_BC_Sum = Fill_BC_Sum + list_D_Satisfy.ToCount(); //整體
                        }
                    ...
                    //string lst1A = "\n";//test //int iRow2 = 0;//test
                    // 小型企業人力提升計畫
                    if (form.PlanTypeCode == "0" || form.PlanTypeCode == "2")
                    {
                        // 取出介接資料，條件：年度+該登入者底下的老師 (分署就是分署底下的老師清單，管理者就是全部老師)
                        Hashtable HtYear = new Hashtable { ["Year"] = form.Year };
                        var list_S_Class = dao.QueryForListAll<TransData_S_Class>("AM.Get_TransData_S_Class_TeachingHours", HtYear).Where(x => tmpTeachers_list.Contains(x.TeacherID)).ToList();
                        ...
                        foreach (var rowD in list_S_Class.Where(x => x.JobAbilityCode == "DC"))
                        {
                            ...
                            var list_S_Satisfy = dao.QueryForListAll<tbTransData_S_Satisfy>("AM.Get_tbTransData_S_Satisfy", HtParms).ToList();
                        ...
                        foreach (var rowD in list_S_Class.Where(x => x.JobAbilityCode == "KC"))
                        {
                            ...
                            //if (tmpUNITID == 1)
                            //{
                            //    foreach (var lst1 in list_S_Satisfy)
                            //    {
                            //        iRow2 += 1;
                            //        lst1A += string.Concat(iRow2, "-", tmpUNITID, "-", lst1.CourseUnitCode, "-", lst1.TeacherID, "-", lst1.TeacherName, "-", lst1.StudentID, "-", lst1.StudentName, "\n");
                            //    }
                            //}
                            //test
                        }
                        //LOG.Debug(string.Concat("##xx-lst1A\n", lst1A));//test
                    ...
                    if (form.PlanTypeCode == "0" || form.PlanTypeCode == "3")
                    {
                        // 取出介接資料，條件：年度+該登入者底下的老師 (分署就是分署底下的老師清單，管理者就是全部老師)
                        Hashtable HtYear = new Hashtable { ["Year"] = form.Year };
                        var list_K_Class = dao.QueryForListAll<TransData_K_Class>("AM.Get_TransData_K_Class_TeachingHours", HtYear).Where(x => tmpTeachers_list.Contains(x.TeacherID)).ToList();
                        ...
                        foreach (var rowD in list_K_Class.Where(x => x.JobAbilityCode == "DC"))
                        {
                            ...
                            var list_K_Satisfy = dao.QueryForListAll<tbTransData_K_Satisfy>("AM.Get_tbTransData_K_Satisfy", HtParms).ToList();
                            //var list_S_Satisfy = dao.GetRowList(new TransData_S_Satisfy() { ClassDateS = rowD.ClassDateS, CourseUnitCode = rowD.CourseUnitCode, TeacherID = rowD.TeacherID }).ToList();
                            // 列出滿意的學員
                    ...
                    var tmpPaAll = Math.Ceiling(DCBCKC_Sum_Good / DCBCKC_Sum_Fill * 100); //整體
                ...
                if (sm.UserInfo.LoginCharacter == "4")
                {
                    ...
                    // 0,1,2 = 大專-補助大專校院辦理就業學程計畫-授課滿意度達70%以上對象表示滿意情形			
                    double tmp_Good = 0;
                    double tmp_Fill = 0;
                    for (int i = 0; i <= 2; i++)
                    {
                    ...
                    // 3,4,5 = 小人提-小型企業人力提升計畫-授課滿意度達70 %以上對象表示滿意情形	
                    tmp_Good = 0;
                    tmp_Fill = 0;
                    for (int i = 3; i <= 5; i++)
                    {
                ...
                string s_FileName1 = $"師資授課情形統計表_{CommonsServices.GetDateTimeNow1()}";
                string s_FileName1_xlsx = string.Concat(s_FileName1, ".xlsx");
                // OK
                FuncLogAdd.Do(ModifyEvent.NONE, ModifyType.PRINT, ModifyState.SUCCESS, "AM/C402M", null, form, s_FileName1 + (form.ExportFormat == "0" ? ".xlsx" : ".ods"));
