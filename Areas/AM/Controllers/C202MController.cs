...
namespace WKEFSERVICE.Areas.AM.Controllers
{
    public class C202MController : WKEFSERVICE.Controllers.BaseController
    {
        ...
        public ActionResult IndexFromHome(string PlanTypeCode, string JobAbilityCode)
        {
            ...
            C202MFormModel form = new C202MFormModel();
        ...
        public ActionResult Export(C202MFormModel form)
        {
            ...
            try
            {
                    ...
                    for (int i = 1; i <= DisplayCols.Length; i++)
                    {
                        ...
                        else if (DisplayFields[i - 1] == "Suggest")
                        {
                            if (row.PlanTypeCode.TONotNullString() == "1")
                            {
                                ...
                                if (tmpList.Any()) objSheet.Cells[Row, i].Value =
                ...
                // OK AM/C202M - 管理者首頁>師資管理>教師授課資料：匯出師資個人授課總表
                FuncLogAdd.Do(ModifyEvent.NONE, ModifyType.PRINT, ModifyState.SUCCESS, "AM/C202M", null, form, s_FileName1_xlsx);
        ...
        [HttpGet]
        public ActionResult Detail(string FilterString, string PlanTypeCode,
            string PlanID, string ClassID, string CourseUnitCode,
        ...
        public ActionResult Detail_Back(C202MViewModel model)
        {
            ...
            if (model.Detail.FilterString.TONotNullString() != "")
            {
                ...
                if (Filters.Any() && Filters.ToCount() == 8)
                {
                    var tmpStr1 = Filters[2].Split(',').Where(x => x != "").ToList();
                    var tmpStr2 = Filters[7].Split(',').Where(x => x != "").ToList();
                    ...
                    model.Form.TeacherUnit = string.Join(",", tmpStr2);
                }
            }
