...
{
    public class C504MViewModel
    {
        public C504MViewModel() { this.Form = new C504MFormModel(); }

        public C504MFormModel Form { get; set; }
    }

    public class C504MFormModel
    {
        public C504MFormModel() { this.ExportFormat = "0"; }
        ...
        //用於 SQL 辨別，無實際參與資料過濾
        /// <summary>文字框，輸入關鍵字，針對姓名欄位做模糊查詢</summary>
        [Display(Name = "教師姓名")]
        [Control(Mode = Control.Hidden)]
        ...
        public IList<SelectListItem> TeacherUnit_list
        {
            get
            {
                ...
                ShareCodeListModel model = new ShareCodeListModel();
                List<SelectListItem> list = model.UNIT_List.Where(x => x.Value == sm.UserInfo.User.UNITID).ToList();  // 分署：鎖定登入者所屬之分署，反灰不可修改
                //list.Insert(0, new SelectListItem() { Text = "全部", Value = "" });  // 分署：鎖定登入者所屬之分署，反灰不可修改
                return list;
            }
        }
