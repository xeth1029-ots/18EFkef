...
namespace WKEFSERVICE.Areas.A3.Models
{
    ...
    public class C101MFormModel : PagingResultsViewModel
    {
        ...
        public IList<SelectListItem> PostTo_list
        {
            get
            {
                ShareCodeListModel model = new ShareCodeListModel();
                var itemAll = new SelectListItem() { Text = "全部", Value = "" };
                var list = model.GRPID_List.Where(x => x.Value.TOInt32() <= 2).ToList();
                list.Insert(0, itemAll);
                return list;
            }
        }
    ...
    public class C101MGridModel : MsgPost
    {
        [NotDBField]
        ...
        public bool isCanModify
        {
            get
            {
                ...
                AMDAO dao = new AMDAO();
                string DataGrp = dao.GetRow(new AMDBURM() { USERNO = this.UpdatedAccount.TONotNullString() }).GRPID;
                return (
                    Convert.ToDateTime(this.PostDateE) >= DateTime.Now.Date &&
                    sm.UserInfo.User.UNITID == this.PublishedUnit.TONotNullString() &&  // 只有 "自己分署的資料" 才可以修改 (發布單位(分署別))
                    DataGrp == "3"  // 且資料同為 "分署使用者" 所建立之資料 才可以修改
                );
            }
        }
    ...
    public class C101MDetailModel : MsgPost
    {
        public C101MDetailModel() { this.IsNew = false; }
        ...
        public IList<SelectListItem> PostTo_list
        {
            ...
            {
                ShareCodeListModel model = new ShareCodeListModel();
                return model.GRPID_List.Where(x => x.Value.TOInt32() <= 2).ToList();
            }
        }
        ...
        public IList<C101MAttachedsModel> Attacheds { get; set; }
    ...
    public class C101MAttachedsModel : MsgPostAttached
    {
        public C101MAttachedsModel()
        {
            this.isActive = true;
