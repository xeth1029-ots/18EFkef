...
{
    public class C205MController : WKEFSERVICE.Controllers.BaseController
    {
        ...
        {
            C205MViewModel model = new C205MViewModel();
            return View(model.Form);
        }

        [HttpGet]
        public ActionResult IndexFromHome(string PlanTypeCode, string JobAbilityCode)
        {
            SessionModel sm = SessionModel.Get();
            C205MFormModel form = new C205MFormModel();
            form.PlanTypeCode = PlanTypeCode;
            form.JobAbilityCode = JobAbilityCode;
            return Index(form);
        }

        [HttpPost]
        public ActionResult Index(C205MFormModel form)
        {
            SessionModel sm = SessionModel.Get();
            A3DAO dao = new A3DAO();
            ...
            // 查詢結果
            form.LoginUnit = sm.UserInfo.User.UNITID;

            form.Grid = dao.QueryC205M(form, form.PlanTypeCode, false);
        ...
        /// <returns></returns>
        [HttpPost]
        public ActionResult Export(C205MFormModel form)
        {
            SessionModel sm = SessionModel.Get();
            ModelState.Clear();
            ActionResult rtn = View("Index", form);
            ...
            A3DAO dao = new A3DAO();
            form.LoginUnit = sm.UserInfo.User.UNITID;
            //A3/C205M -分署首頁>師資管理>教師授課資料：匯出師資個人授課總表 
            form.Grid = dao.QueryC205M(form, form.PlanTypeCode, true);
            ...
            try
            {
                ...
                //授課對象針對師資授課表示滿意之人數達70%以上 "授課滿意度達70%以上對象表示滿意情形",
                ...
                // 老師清單抓進來，暫存查詢用
                //var wlist = (form.TeacherUnit != null && form.TeacherUnit.Length == 1) ? new Teacher() { UnitCode = form.TeacherUnit } : new Teacher() { };
                var wlist = new Teacher() { UnitCode = sm.UserInfo.User.UNITID };
                var lstTeachers = dao.GetRowList(wlist).ToList();
                // 開始塞資料
                ...
                foreach (var row in form.Grid)
                {
                    for (int i = 1; i <= DisplayCols.Length; i++)
                    {
                        ...
                        {
                            if (row.PlanTypeCode.TONotNullString() == "1")
                            {
                            ...
                            else if (row.PlanTypeCode.TONotNullString() == "2")
                            {
                                var tmpList = dao.GetRowList(new TransData_S_Satisfy()
                                {
                                    TrainID = row.PlanID,
                                    ClassID = row.ClassID,
                                    CourseUnitCode = row.CourseUnitCode,
                        ...
                        else if (DisplayFields[i - 1] == "TeacherUnitCode" && lstTeachers.Any())
                        {
                            var tmpTList = lstTeachers.Where(x => x.IDNO == row.TeacherID.TONotNullString()).FirstOrDefault();
                            if (tmpTList != null && tmpTList.UnitCode.TONotNullString() != "")
                            {
                                objSheet.Cells[Row, i].Value = sclm.UNIT_All_List.Where(x => x.Value == tmpTList.UnitCode).FirstOrDefault().Text;
                            }
                        }
                ...
                // OK A3/C205M -分署首頁>師資管理>教師授課資料：匯出師資個人授課總表 
                FuncLogAdd.Do(ModifyEvent.NONE, ModifyType.PRINT, ModifyState.SUCCESS, "A3/C205M", null, form, s_FileName1_xlsx);
            ...
            string Year, string ClassDateS, string TeacherID)
        {
            SessionModel sm = SessionModel.Get();
            C205MViewModel model = new C205MViewModel();
            model.Detail = new C205MDetailModel();
            model.Detail.FilterString = FilterString;
            ActionResult rtn = View("Detail", model);
            //PlanTypeCode, 1:補助大專校院辦理就業學程計畫, 2:小型企業人力提升計畫, 3:企業人力資源提升計畫(大人提)
            ...
            if (PlanTypeCode == "1")
            {
                ...
                {
                    // 問卷填答人數
                    model.Detail.FillStudentNum = tmpList.ToCount();
                    // 授課滿意度達70%以上對象表示滿意情形
                    ...
                    model.Detail.Satisfy = string.Concat(Math.Ceiling(totalSatisfy * 100), "%");
                    // 建議事項(意見或建議)
                    model.Detail.Suggest = string.Join("\r\n", tmpList.Select(x => x.Suggest).Where(x => x.TONotNullString() != "").ToList());
                }
            }
            else if (PlanTypeCode == "2")
            {
                // 小型企業人力提升計畫
            ...
            else if (PlanTypeCode == "3")
            {
                // 3:企業人力資源提升計畫(大人提)
                var data = dao.GetRow(new TransData_K_Class()
                {
                    TrainID = PlanID,
                    ClassID = ClassID.TOInt32(),
                    ClassDateS = ClassDateS,
                    CourseUnitCode = CourseUnitCode,
                    TeacherID = TeacherID,
                });
                model.Detail.InjectFrom(data);
                // 其他額外欄位
                var tmpList = dao.GetRowList(new TransData_K_Satisfy()
                {
                    TrainID = PlanID,
                    ClassID = ClassID.TOInt32(),
                    CourseUnitCode = CourseUnitCode,
        ...
        [HttpPost]
        public ActionResult Detail_Back(C205MViewModel model)
        {
            SessionModel sm = SessionModel.Get();
            // 處理 查詢頁的 查詢條件
            if (model.Detail.FilterString.TONotNullString() != "")
            {
                ...
                if (Filters.Any() && Filters.ToCount() == 7)
                {
                    ...
                    model.Form.TeacherName = Filters[6];
                }
            }
