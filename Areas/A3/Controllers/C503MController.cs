...
{
    public class C503MController : WKEFSERVICE.Controllers.BaseController
    {
        [HttpGet]
        public ActionResult Index()
        {
            SessionModel sm = SessionModel.Get();
            C503MViewModel model = new C503MViewModel();
            model.Form.TeacherUnit = sm.UserInfo.User.UNITID;
            return View(model.Form);
        }

        [HttpPost]
        public ActionResult Index(C503MFormModel form)
        {
            SessionModel sm = SessionModel.Get();
            ModelState.Clear();
            ...
            A3DAO dao = new A3DAO();
            // 暫存資料 老師資料表
            var lstTeachers = dao.GetRowList(new Teacher() { Online = "Y", UnitCode = form.TeacherUnit }).ToList();
            ...
            try
            {
                ...
                if (form.PlanTypeCode == "0" || form.PlanTypeCode == "1")
                {
                    ...
                    var mainDatas_D = dao.QueryC503M1(form.Year.TONotNullString());
                    ...
                    if (mainDatas_D.Any())
                    {
                        ...
                        {
                            // 老師資料
                            var findTeacher = lstTeachers.Where(x => x.IDNO == row.TeacherID).FirstOrDefault();
                ...
                if (form.PlanTypeCode == "0" || form.PlanTypeCode == "2")
                {
                    ...
                    var mainDatas_S = dao.QueryC503M2(form.Year.TONotNullString());
                    //if (form.TeacherUnit.TONotNullString() != "" && form.TeacherUnit.TONotNullString() != "0")
                ...
                if (form.PlanTypeCode == "0" || form.PlanTypeCode == "3")
                {
                    ...
                    // 產生標題列
                    objSheet.Cells[1, 1].Value = (form.Year.TOInt32() - 1911).ToString() + " 年度關鍵就業力課程滿意度調查-企業人力資源提升計畫";
                    objSheet.SelectedRange[1, 1, 1, 27].Merge = true;
                    objSheet.Cells[2, 1].Value = "講師姓名";
                    objSheet.Cells[2, 2].Value = "所屬轄區";
                    objSheet.Cells[2, 3].Value = "授課內容所署分署";
                    ...
                    // 開始填資料
                    //var mainDatas_S = dao.GetRowList(new TransData_S_Satisfy() { Year = form.Year.TONotNullString() });
                    var mainDatas_K = dao.QueryC503M3($"{form.Year}");
                    //if (form.TeacherUnit.TONotNullString() != "" && form.TeacherUnit.TONotNullString() != "0")
                    //    mainDatas_S = mainDatas_S.Where(x => x.UnitCode == form.TeacherUnit.TONotNullString()).ToList();
                    int iRow = 2;
                    if (mainDatas_K.Any())
                ...
                // OK
                FuncLogAdd.Do(ModifyEvent.NONE, ModifyType.PRINT, ModifyState.SUCCESS, "A3/C503M", null, form, s_FileName1 + (form.ExportFormat == "0" ? ".xlsx" : ".ods"));
