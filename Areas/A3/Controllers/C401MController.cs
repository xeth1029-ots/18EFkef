...
namespace WKEFSERVICE.Areas.A3.Controllers
{
    public class C401MController : WKEFSERVICE.Controllers.BaseController
    {
        ...
        public ActionResult Index()
        {
            ...
            // 預設勾選登入分署，但可勾選其他分署查詢。
            if (model.Form.CreatedUnit.TONotNullString() == "")
        ...
        public ActionResult Index(C401MFormModel form)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                form.MeetingType = "2";
                // 預設勾選登入分署，但可勾選其他分署查詢。
                if (form.CreatedUnit.TONotNullString() == "")
        ...
        public ActionResult Detail(string Seq)
        {
            ...
            C401MViewModel model = new C401MViewModel();
            model.Detail = new C401MDetailModel();
            model.Detail = dao.DetailC401M(Seq.TONotNullString());
            ...
            model.Detail.Attacheds = dao.DetailAttachedsC401M(Seq.TONotNullString());
        ...
        public ActionResult DetailSignUp(string Seq, string vPage)
        {
            ...
            ModelState.Clear();
            A3DAO dao = new A3DAO();
            C401MViewModel model = new C401MViewModel() { DetailSignUp = new C401MDetailSignUpModel() };
            ...
            model.DetailSignUp = dao.DetailSignUpC401M_Mst(Seq.TONotNullString());
            if (model.DetailSignUp == null || model.DetailSignUp.MeetingType != "2")
            {
                sm.LastErrorMessage = "查無資料";
                return Index(model.Form);
            }
            ...
            model.DetailSignUp.SignUpList = dao.DetailSignUpC401M_Det(iSeq, iMINROW, iMAXROW);
            return View("DetailSignUp", model);
        }

        [HttpPost]
        public ActionResult DetailAttachedsUpload(C401MViewModel model)
        {
            SessionModel sm = SessionModel.Get();
            A3DAO dao = new A3DAO();
            ...
            {
                // 上傳檔案
                string tmpPath = Server.MapPath(new AMDAO().UploadPathC301M);  // 資料夾路徑名
                ...
                try
                {
                    ...
                    FuncLogAdd.Do(ModifyEvent.AFTER, ModifyType.CREATE, (keyid >= 1) ? ModifyState.SUCCESS : ModifyState.FAIL, "A3/C401M", Ins, null, "[MeetingAttached]");
                    dao.CommitTransaction();
                    #endregion
                    #region Model Insert
                    model.Detail._UPLOAD_FILE = null;
                    if (model.Detail.Attacheds == null) model.Detail.Attacheds = new List<C401MAttachedsModel>();
                    model.Detail.Attacheds.Add(new C401MAttachedsModel()
                    {
                ...
                catch (Exception ex)
                {
                    ...
                    if (System.IO.File.Exists(tmpPath + "/" + tmpFileName))
                    {
                        ...
                        FileLogAdd.Do(ModifyType.DELETE, ModifyState.SUCCESS, "A3/C401M", file.FileName, tmpFileName);
                    }
        ...
        [HttpPost]
        public ActionResult Save(C401MDetailModel detail)
        {
            SessionModel sm = SessionModel.Get();
            A3DAO dao = new A3DAO();
            ...
            if (ModelState.IsValid)
            {
                ...
                detail._tmpPicPath = Server.MapPath(new AMDAO().UploadPathC301M);  // 資料夾路徑名
                dao.InsertOrUpdateC401M(detail);
            ...
            return (detail.IsNew) ? Index(new C401MFormModel()) : Detail(detail.Seq.TONotNullString());
        }

        [HttpPost]
        public ActionResult SaveSignUp(C401MDetailSignUpModel detailSignUp)
        {
            if (detailSignUp.SignUpList.ToCount() > 0)
            {
                A3DAO dao = new A3DAO();
                dao.InsertOrUpdateC401M_SignUp(detailSignUp);
                SessionModel sm = SessionModel.Get();
                sm.LastResultMessage = "資料已儲存";
            }
        ...
        [HttpPost]
        public ActionResult Delete(C401MFormModel form)
        {
            SessionModel sm = SessionModel.Get();
            A3DAO dao = new A3DAO();
            int i_Seq = 0;
            ...
            // 刪除 檔案
            string FilePath = Server.MapPath(new AMDAO().UploadPathC301M);
            #region Meeting
            ...
            if (tmpData.PicName.TONotNullString() != "")
            {
                ...
                if (System.IO.File.Exists(FullFileName))
                {
                    ...
                    FileLogAdd.Do(ModifyType.DELETE, ModifyState.SUCCESS, "A3/C401M", tmpData.PicName.TONotNullString(), null);
                }
            }
            #endregion
            ...
            foreach (var itm in tmpDataAtta)
            {
                ...
                {
                    System.IO.File.Delete(FullFileName);
                    FileLogAdd.Do(ModifyType.DELETE, ModifyState.SUCCESS, "A3/C401M", itm.FileNameOrg.TONotNullString(), itm.FileNameNew.TONotNullString());
                }
            }
            #endregion
            // 刪除 資料
            dao.DeleteC401M(form.Seq_forDel.TONotNullString(), sm.UserInfo.UserNo);
            sm.LastResultMessage = "已刪除";
            return Index(form);
        }

        [HttpPost]
        public ActionResult TeacherSearch(C401MViewModel model)
        {
            ModelState.Clear();
            A3DAO dao = new A3DAO();
            SessionModel sm = SessionModel.Get();
            model.DetailSignUp.TeacherSearchList = dao.GetTeacherListC401M(
                model.DetailSignUp.TeacherSearch.TONotNullString(),
                sm.UserInfo.User.UNITID
            );
            return PartialView("_TeacherPopupRows", model);
        }

        [HttpPost]
        public ActionResult TeacherSearchSelect(C401MViewModel model)
        {
            ModelState.Clear();
            SessionModel sm = SessionModel.Get();
            if (model.DetailSignUp.TeacherSearchList.ToCount() > 0)
                foreach (var item in model.DetailSignUp.TeacherSearchList)
                {
                    ...
                    if (model.DetailSignUp.SignUpList == null) model.DetailSignUp.SignUpList = new List<C401MDetailSignUpListModel>();
                    ...
                    // 未找到重複時，則新增
                    C401MDetailSignUpListModel NewItem = new C401MDetailSignUpListModel();
        ...
        [HttpPost]
        public ActionResult TeacherSearchSelectRemoveRow(C401MViewModel model)
        {
            ModelState.Clear();
