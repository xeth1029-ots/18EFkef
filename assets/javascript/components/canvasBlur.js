/**
 * @param {String|Element} source 原始圖像資源，可以是 <IMG>|<canvas> DOM 元素，也可以是 URL 字符串
 * @param {Number} blur 模糊的半徑大小
 * @param {Array} bounding 模糊的半徑位置，可選，如果不設置，則認為是整體模糊
 * @param {Element|Function} output 顯示模糊圖像的 canvas 元素，或者是回調函數（參數是canvas）
 * @returns 模糊後的 canvas 元素
 */
var localBlur = function(source, radius, bounding, output) {
  ...
  var eleOutput = output;
  eleOutput.className = "asd"
  var contextBlur = eleOutput.getContext('2d');
  console.log(contextBlur)
  // 資源的寬高尺寸
  var width = 0;
  var height = 0;


  var ctx = eleOutput.getContext('2d'),
    img = eleSource,
    play = false;



  // img.src = eleSource.src;
  eleOutput.className = "asd"
  // wait until image is actually available
  // eleOutput.onload = pixelate;
  // 模糊繪製的主方法
  ...
  var pixelate = function(v) {
    width = eleSource.width;
    height = eleSource.height;
    console.log(eleOutput);
    eleOutput.width = width;
    eleOutput.height = height;
    var size = 0.06 * 0.1;
    ...
    console.log(w , h);
    console.log(eleSource , height);
    // draw original image to the scaled size
    // ctx.drawImage(img, 0, 0, width, height);
    // console.log(ctx.drawImage( 0, 0, w, h, 0, 0, width, height));
    // then draw that scaled image thumb back to fill canvas
    // As smoothing is off the result will be pixelated
    ctx.drawImage(eleOutput, 0, 0, w, h, 0, 0, width, height);
