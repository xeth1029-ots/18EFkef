...
/*
 *[功能描述]
 * 瀏覽器支援：IE 9+、FireFox 3.0+、Chrome 4.0+、Safari 4.0+、iPhone 2.0+、Android 2.0+
 * 给不支持工作階段存儲的浏览器创建一个 window.sessionStorage 对象来提供类似接口
 * 该对象支持以下方法或属性
 ...
 * 二次包装的接口 window.LS 提供以下方法和属性（如果有jQuery则同样会扩展该对象），推荐使用
	...
	obj : function() 返回一个对象描述的sessionStorage副本
	length : int
 * 
 *[已知问题、使用限制]
 * 原生工作階段存儲的key是区分大小写的，模拟对象不区分（因为userData不区分key的大小写）
 * 模拟对象的 clear 方法仅仅能清理通过本组件设定的数据
 * 模拟对象的实际的存儲容量跟原生工作階段存儲有差异
 * 模拟对象不支持任何sessionStorage事件件
 *
 *[更新日志]
 * 2012-06-20 马超 创建
 * 2012-06-27 马超 增加clear相关方法和属性
 * 2012-07-02 马超 修改节点存儲方式
 * 2012-07-03 马超 增加二次包装以优化接口使用
 * 2012-07-04 马超 修改内部逻辑，取消原有的单独存儲key的方案，修改查询不存在key的时候的默认值为undefined
 * 2012-07-05 马超 增加二次包装的obj方法
 * 2013-03-06 胡志明 基于瑞星的刘瑞明提供的方案，兼容360急速浏览器IE模式（IE6），降低浏览器自带sessionStorage优先级，优先使用userData。
 * 2013-03-11 胡志明 恢复iframe代理创建head标签用户存儲数据，修正了userData无法垮目录读写问题
 * 2013-03-14 胡志明 对部分无法创建iframe代理的浏览器尝试使用自带sessionStorage，如果不自带则暂时不支持工作階段存儲
 * 2013-03-18 马超 优化加载判断逻辑和代码，以最大限度保证组件可用
 * 2013-04-23 马超 增加更多的错误处理，进一步提高浏览器的兼容性
 * 2013-05-04 马超 增加对userData的key的无效字符的转义处理功能
 * 2013-06-06 马超 优先探测工作階段存儲，解决IE9下userData使用问题（刷新页面后无效）
 */
(function(window){
	//准备模拟对象、空函数等
	...
	//优先探测userData是否支持，如果支持，则直接使用userData，而不使用sessionStorage
	//以防止IE浏览器关闭sessionStorage功能或提高安全级别(*_* 万恶的IE)
	//万恶的IE9(9.0.11）)，使用userData也会出现类似seesion一样的效果，刷新页面后设置的东西就没有了...
	//只好优先探测工作階段存儲，不能用再尝试使用userData
	(function(){
		// 先探测工作階段存儲 2013-06-06 马超
		// 尝试访问工作階段存儲，如果访问被拒绝，则继续尝试用userData，注： "sessionStorage" in window 却不会返回权限错误
		...
		if( "sessionStorage" in window){
			try {
				...
				//如果报错，说明浏览器已经关闭了工作階段存儲或者提高了安全级别
				//则尝试使用userData
			}
		}
		
		//继续探测userData
		var o = document.getElementsByTagName("head")[0], hostKey = window.location.hostname || "sessionStorage", d = new Date(), doc, agent;
		
		//typeof o.addBehavior 在IE6下是object，在IE10下是function，因此这里直接用!判断
		//如果不支持userData则跳出使用原生sessionStorage，如果原生sessionStorage报错，则放弃工作階段存儲
		if(!o.addBehavior){
			try{
                LS = window.sessionStorage;
			}catch(e){
		...
		try{ //尝试创建iframe代理，以解决跨目录存儲的问题
			...
			//这里通过代理document创建head，可以使存儲数据垮文件夹访问
			o = doc.createElement('head');
			doc.appendChild(o);
		}catch(e){
			//不处理跨路径问题，直接使用当前页面元素处理
			//不能跨路径存儲，也能满足多数的工作階段存儲需求
			//2013-03-15 马超
		...
		LS= {
			...
			setItem: function(key, value){
				...
				}catch(e){//这里IE9经常报没权限错误,但是不影响数据存儲
				}
			},
			removeItem: function(key){
	...
	window.LS = !LS ? notSupport : {
		...
		//工作階段存儲数据遍历，callback接受两个参数 key 和 value，如果返回false则终止遍历
