...
function unblockUI_PRT() {
    var timeLog = 3000;

    setTimeout(function () {
        ajaxLoadMore('/Ajax/RunRPT_ON', null, function (retData) {

            if (retData.status) { unblockUI(); } else { unblockUI_PRT(); }
        });
    }, timeLog);
}
/*用來顯示操作訊息*/
...
function blockAlert(msg, title, sureCallback, height) {
    ...
    $.confirm({
        ...
        content: "<div class='blockAlertMessage' style='padding-top:5px;width:100%;height:" + wdHeight + ";background-color:#fcfcfc;overflow:auto;word-wrap:break-word;white-space:normal;line-height:15px'>" + msg.replace(/(\n)+/g, '<br />') + "</div>",
        theme: 'white',
...
function ajaxUploadFile(url, fileElement, jsonParms, successCallback, errorCallback, isTtraditional, notAutoBlock) {
    try {
        ...
        if (!fieldName || $.trim(fieldName).length == 0)
        {fieldName = "file";}
