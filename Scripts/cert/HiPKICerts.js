...
function _sendHttpRequestIE(target, data) {
    if (console) console.log("_sendHttpRequestIE: " + target);

    var http = document.getElementById("http");
    if (!http || !http.sendRequest) {
        if (console) console.error("Browser does NOT support application/x-httpcomponent HTTP object");
        return null;
    }
    http.url = target;
    ...
    if (code != 0) {
        if (console) console.error("http.sendRequest faild, return code: " + code);
        return null;
    }
...
function _checkFinish() {
    if (_postTarget) {
        ...
        if (_validationCallback && _validationCallback instanceof Function) {
            _validationCallback(null, -9999, MSG_NotInstallPKILocalServer);
        }
        else {
...
function CertValidation(userPIN, callback) {
    ...
    globalSetting.processingImg = "/images/waiting.gif";
    var img = new Image();
    img.src = globalSetting.processingImg;
            ...
            if (rtnCode == 0) {
                ...
                _getUserCert(function (rtnCode, msg) {
                    ...
                    if (rtnCode == 0) {
                        // 讀取憑證資料成功
                        if (console) {
                            console.log("Read Certifacation Success ");
                        ...
                        if (console) console.error("Read Certifacation Failed: " + msg);
                ...
                // 簽章失敗(PIN 碼驗證失敗)
                if (console) console.error("VERIFY Failed: " + msg);
...
function _makeSignature(userPIN, callback) {
    ...
    {
        //_postTarget = window.open(HiPKULocalServer + "/waiting.gif", "HiPKIwin", "height=200, width=200, left=100, top=20");
        var data = _sendHttpRequestIE(HiPKULocalServer + "/sign", "tbsPackage=" + _getTbsPackage() );
        //_postTarget.close();
        //_postTarget = null;

        if (!data) {
            if (callback && callback instanceof Function) {
                callback(null, -9999, MSG_NotInstallPKILocalServer);
            }
            else {
                alert(MSG_NotInstallPKILocalServer);
            }
...
function _getUserCert( callback ) {
    ...
    {
        //_postTarget = window.open(HiPKULocalServer + "/waiting.gif", "HiPKIwin", "height=200, width=200, left=100, top=20");
        var data = _sendHttpRequestIE(HiPKULocalServer + "/pkcs11info?withcert=true", "");
        //_postTarget.close();
        //_postTarget = null;
...
function _triggerMakeSignCallback(signedData, rtnCode, msg) {
    if (_makeSignCallback) {
        ...
        if(console) console.log("_makeSignCallback undefied!");
    }
}
...
function _triggerGetCertCallback(certData, rtnCode, msg) {
    if (console) console.log(certData);

    if (_getCertCallback) {
        _getCertCallback(certData, rtnCode, msg);
    }
    else {
        if (console) console.log("_getCertCallback undefied!");
    }
}
...
function _setUserCert(certData) {
    ...
    for (var index in slots) {
        ...
        for (var indexCert in certs) {
            if(console) console.log(certs[indexCert].label
...
function _receiveMessage(event) {
    ...
    if (event.origin != HiPKULocalServer) {
        if (console) console.debug("_receiveMessage: event.origin != " + HiPKULocalServer + " Ignored");
        return;
    }
    try {
        if (event.data && event.data.length > 0) {
            ...
            if (ret.func) {
                if (console) console.debug("_receiveMessage: event.data.func = " + ret.func);

                if (ret.func == "getTbs") {
                    clearTimeout(_timeoutId);

                    var json = _getTbsPackage();
                    //owasp 4AF21AA29700BACD7DB13DD8D63FBDEBE7EC430CD537F4C2C4DC9D951205478C624A08B0492A211199E59E846E2A739D
                }
                ...
                if (console) console.error("_receiveMessage(): event has no func");
            }
        }
        else {
            if (console) console.error("_receiveMessage(): event has empty data");
        }
        
    } catch (e) {
        //errorhandle
        if (console) {
            console.error("_receiveMessage() exception:");
            console.error(e);
        }
    }
}
...
function onLoadInit() {
    if (_UA.indexOf("MSIE") != -1 || _UA.indexOf("Trident") != -1) {
        ...
        if (console) console.log("HiPKICerts onLoadInit: IE mode");
        _isIE = true;
        _createHttpObject();
    }
}
